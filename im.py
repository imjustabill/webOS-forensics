...
for text, first, last, status, address, timeStamp, flag in c.execute('''
 SELECT com_palm_pim_FolderEntry.messageText,
    ...
    WHERE messageType = "IM"
    ORDER BY com_palm_pim_FolderEntry.timeStamp'''):

	# find the name if all we have is the screen name	
	if first == None and last == None:
		d = conn.cursor()
		d.execute('''SELECT firstName, lastName
			 FROM com_palm_pim_Contact 
			 WHERE id = 
				(SELECT com_palm_pim_Contact_contactPts_id 
				FROM com_palm_pim_ContactPoint WHERE type = "IM" 
				AND value = "''' + address + '" LIMIT 1) LIMIT 1')
		row = d.fetchone()
		if row == None:
                        first = "Unknown"
                        last = "Unknown"
